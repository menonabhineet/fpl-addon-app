...
export default async function DashboardPage({ 
  ...
  const [
    ...
    supabase.from('team_predictions').select('*').eq('user_id', user.id).order('gameweek_id', { ascending: true }).range(0, 9999),
    supabase.from('score_predictions').select('*').eq('user_id', user.id).range(0, 9999),
